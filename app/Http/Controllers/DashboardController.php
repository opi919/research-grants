...
class DashboardController extends Controller
{
    public function index()
    {
        $stats = [
            ...
            'nih_grants' => Cache::remember('nih_grants', 60 * 24 * 30, function () {
                return ResearchGrant::where('source', 'csv1')->count();
            }),
            'excel_grants' => Cache::remember('excel_grants', 60 * 24 * 30, function () {
                return ResearchGrant::where('source', 'csv2')->count();
            }),
            'nsf_grants' => Cache::remember('nsf_grants', 60 * 24 * 30, function () {
                return ResearchGrant::where('source', 'json')->count();
            }),
        ];

        $recent_grants = Cache::remember('recent_grants', 60 * 24 * 30, function () {
            return ResearchGrant::orderBy('created_at', 'desc')
                ->paginate(10);
        });

        $top_institutions = Cache::remember('top_institutions', 60 * 24 * 30, function () {
